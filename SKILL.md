...
当用户显式提到 `$ssh-skill <参数>` 时，根据参数执行对应操作；未显式提到时，只要用户请求 SSH/远程服务器操作，也应自动使用本 skill。
...
ssh_execute.py 会自动检测守护进程：简单命令有则走长连接（~0.12s），无则自动启动守护进程；`--stdin` / `--script-file` 会先上传到远端临时文件再执行。

### 复杂命令与多行脚本

当远程命令包含以下内容时，**不要**直接写成 `ssh_execute.py <别名> "<命令>"`：

- `$()`、反引号、`${VAR}` 等会被本地 shell 先处理的展开
- `cat <<EOF` / heredoc
- 写入 `.env`、证书、私钥、JSON、YAML 等需要原样落盘的内容
- 很长的 `docker compose` 覆盖命令或多行部署脚本

这类场景必须优先使用 `--stdin` 或 `--script-file`，让脚本内容原样传到远端再执行。

如需指定解释器，请在脚本第一行写 shebang，例如 `#!/usr/bin/env bash` 或 `#!/usr/bin/env python3`。
...
- 简单只读命令可直接用 `"<命令>"`
- 只要出现多层引号、shell 展开、heredoc、长脚本，就切换到 `--stdin` 或 `--script-file`
- 不要为了临时覆盖容器启动命令，把很长的 Python 或 shell 片段硬塞进 `docker compose` 的 `command`
...
- 含 shell 展开、heredoc、密钥或多行脚本的远程操作，必须使用 `ssh_execute.py --stdin` 或 `--script-file`
